...
#echo $Home
awk -v FILE_PREFIX="$file_prefix" -v FILE="$file" -v VERSION="$version" -v do_IBRAV=$do_ibrav -v SG=$sg -v home=$Home '
...
#
# We treat the text between two single apostrophes as a string
# returns the field-th field of the str string
# If the two apostrophes are not present, \47 returns the field-th field
#
 idx1=0         
 str=str " "
 while (str) {
...
#
# In str replaces all occurrences of pat with the value repl e.g.:
...
END {
 ...
 print "                 calculation = \x27" "relax\x27"
 print "                restart_mode = \x27" "from_scratch\x27"
 print "                      outdir = \x27" "./Outdir\x27"
 print "                  pseudo_dir = \x27"home "\x27"
...
if (SG==2) {
 ...
 print "                   degauss   = 0.01 "
 print "                    smearing = \x27" "gaussian \x27" 
 print " !                     nspin = 2 "
 print " ! starting_magnetization(1) = 1 "
 print " ! starting_magnetization(2) = 0 "
 print " ! starting_magnetization(3) = 0 "
 print " /"
 print " &ELECTRONS"
 ...
 print " /"
 print "\nATOMIC_SPECIES"
 for (i=0; i<ntyp; i++) printf "  %3s  %14.10f  %s.upf\n", AtomTyp[i], Atoms[AtomTyp[i]], AtomTyp[i];
